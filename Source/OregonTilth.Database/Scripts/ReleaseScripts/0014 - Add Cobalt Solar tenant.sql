...
SET @MigrationName = '0014 - Add Cobalt Solar tenant'
...
BEGIN
    ...
    DECLARE @CobaltSolarID int;
    INSERT INTO dbo.Tenant([Name], SubDomain)
    SELECT 'Cobalt Solar', 'cobalt-solar'
    SET @CobaltSolarID = @@IDENTITY
    ...
    SELECT @CobaltSolarID, R.RoleName, R.RoleDescription, R.IsSystemRole, R.Rights, R.Flags, GETUTCDATE(), @MikeyUserID
    FROM dbo.[Role] R
    WHERE R.TenantID = @ESATenantID -- Copy ESA's Roles into Cobalt Solar

    -- Add ESA users to Cobalt Solar tenant.
    INSERT INTO dbo.TenantUser(TenantID, UserID, RoleID, CreateDate, CreateUserID)
    SELECT @CobaltSolarID, U.UserID, CS_R.RoleID, GETUTCDATE(), @MikeyUserID
    FROM dbo.[User] U
    JOIN dbo.[Role] DC_R    ON DC_R.RoleID = U.RoleID          AND DC_R.TenantID = @DeltaConveyanceTenantID
    JOIN dbo.[Role] CS_R    ON CS_R.RoleName = DC_R.RoleName   AND CS_R.TenantID = @CobaltSolarID
    WHERE Email like '%@esassoc.com'
    
    -- Add Tenant Modules
    INSERT INTO dbo.TenantModule(TenantID, ModuleID, DisplayOrder)
    SELECT @CobaltSolarID, 1, NULL
    UNION ALL
    SELECT @CobaltSolarID, 3, 1

    -- Add Field Definitions to Cobalt Solar 
    INSERT INTO dbo.FieldDefinition(TenantID, FieldDefinitionTypeID, FieldDefinitionValue)
    SELECT @CobaltSolarID, FD.FieldDefinitionTypeID, FD.FieldDefinitionValue
    FROM dbo.FieldDefinition FD WHERE FD.TenantID = @DeltaConveyanceTenantID

    -- Add CustomRichTexts to Cobalt Solar 
    INSERT INTO dbo.CustomRichText(TenantID, CustomRichTextTypeID, CustomRichTextContent)
    SELECT @CobaltSolarID, CRT.CustomRichTextTypeID, CRT.CustomRichTextContent
    FROM dbo.CustomRichText CRT WHERE CRT.TenantID = @DeltaConveyanceTenantID

    INSERT INTO dbo.DatabaseMigration(MigrationAuthorName, ReleaseScriptFileName, MigrationReason)
    SELECT 'Mikey Knowles', @MigrationName, 'Add Cobalt Solar tenant.'
END