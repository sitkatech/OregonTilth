...
namespace OregonTilth.API.Services
{
    public class SitkaSmtpClientService
    {
        ...
        public SitkaSmtpClientService(FrescaConfiguration frescaConfiguration)
        {
            _frescaConfiguration = frescaConfiguration;
        }

        /// <summary>
        /// Sends an email including mock mode and address redirection  <see cref="FrescaConfiguration.SITKA_EMAIL_REDIRECT"/>, then calls onward to <see cref="SendDirectly"/>
        /// </summary>
        ...
        /// <summary>
        /// Sends an email message at a lower level than <see cref="Send"/>, skipping mock mode and address redirection  <see cref="FrescaConfiguration.SITKA_EMAIL_REDIRECT"/>
        /// </summary>
        /// <param name="mailMessage"></param>
        public void SendDirectly(MailMessage mailMessage)
        {
            //if (!string.IsNullOrWhiteSpace(FrescaConfiguration.MailLogBcc))
            //{
            ...
            var smtpClient = new SmtpClient(_frescaConfiguration.SMTP_HOST, _frescaConfiguration.SMTP_PORT);
            smtpClient.Send(mailMessage);
            //_logger.Info($"Email sent to SMTP server \"{smtpClient.Host}\", Details:\r\n{humanReadableDisplayOfMessage}");
        }

        /// <summary>
        /// Alter message TO, CC, BCC if the setting <see cref="FrescaConfiguration.SITKA_EMAIL_REDIRECT"/> is set
