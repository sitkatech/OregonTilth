...
namespace OregonTilth.API.Models.HealthMonitor
{
    public class HealthCheckResults
    {
        ...
        public static HealthCheckStatus GetAggregateHealthCheckStatus(List<HealthCheckStatus> healthCheckStatuses)
        {
            ...
            throw new Exception($"Unanticipated combination of HealthCheckStatuses: {badCheckStatuesString}");
        }
        ...
        {
            var allHealthCheckStatuses = Enum.GetValues(typeof(HealthCheckStatus)).Cast<HealthCheckStatus>().ToList();
            ...
            switch (currentStatus)
            {
                ...
                default:
                    throw new Exception($"Unhandled Health Check Status: {currentStatus}");
            }
        }
        ...
        {
            var allHealthCheckStatuses = Enum.GetValues(typeof(HealthCheckStatus)).Cast<HealthCheckStatus>().ToList();
            return string.Join(" ", allHealthCheckStatuses.Select(GetHealthCheckResultsAsPerformanceDataLongPerStatus));
        }
