...
SET @MigrationName = '0038 - Add lv tenant BCN-272'
...
BEGIN
    ...
    DECLARE @LosVaquerosID int;
    INSERT INTO dbo.Tenant([Name], SubDomain)
    SELECT 'Los Vaqueros', 'losvaqueros'
    SET @LosVaquerosID = @@IDENTITY
    ...
    SELECT @LosVaquerosID, R.RoleName, R.RoleDescription, R.IsSystemRole, R.Rights, R.Flags, GETUTCDATE(), @StewartUserID
    FROM dbo.[Role] R
    ...
    SELECT @LosVaquerosID, U.UserID, CS_R.RoleID, GETUTCDATE(), @StewartUserID
    FROM dbo.[User] U
    JOIN dbo.[Role] DC_R    ON DC_R.RoleID = U.RoleID          AND DC_R.TenantID = @DeltaConveyanceTenantID
    JOIN dbo.[Role] CS_R    ON CS_R.RoleName = DC_R.RoleName   AND CS_R.TenantID = @LosVaquerosID
    WHERE Email like '%@esassoc.com'
    
    -- Add Tenant Modules
    INSERT INTO dbo.TenantModule(TenantID, ModuleID, DisplayOrder)
    SELECT @LosVaquerosID, 1, NULL
    UNION ALL
    SELECT @LosVaquerosID, 2, 1
    UNION ALL
    SELECT @LosVaquerosID, 5, 2

    -- Add Field Definitions to Prologis
    INSERT INTO dbo.FieldDefinition(TenantID, FieldDefinitionTypeID, FieldDefinitionValue)
    SELECT @LosVaquerosID, FD.FieldDefinitionTypeID, FD.FieldDefinitionValue
    ...
    SELECT @LosVaquerosID, CRT.CustomRichTextTypeID, CRT.CustomRichTextContent
    ...
    SELECT @LosVaquerosID, NEWID(), 'Environmental Commitment'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Mitigation Measure'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Permit Condition'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Standard Construction Measure'
           
           
    INSERT INTO dbo.ResourceCategory(TenantID, ResourceCategoryID, [Name])
    SELECT @LosVaquerosID, NEWID(), 'Noise and Vibration'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Air Quality'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Biological Resources'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Hydrology and Water Quality'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Cultural Resources'
    UNION  
    SELECT @LosVaquerosID, NEWID(), 'Tribal Cultural Resources'
        
    --Add Phases
    INSERT INTO dbo.[Phase](TenantID, [PhaseID], [Name])
    SELECT @LosVaquerosID, NEWID(), T.[Name]
    FROM dbo.[Phase] T 
    ...
    SELECT 'Stewart Gordon', @MigrationName, '0038 - Add lv tenant BCN-272'
END